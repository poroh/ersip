...
reply_totag_autogenerated_test() ->
    ...
    ?assert(ersip_parser_aux:check_token(iolist_to_binary(ersip_hdr_fromto:assemble(ToTag)))),
    ok.
...
-dialyzer({nowarn_function, userdata_error_on_empty_test/0}).
userdata_error_on_empty_test() ->
    SipMsg = default_sipmsg(),
    SipMsg1 = ersip_sipmsg:set_user_data(my_data_2, SipMsg),
    ?assertEqual(my_data_2, ersip_sipmsg:user_data(SipMsg1)),
    SipMsg2 = ersip_sipmsg:clear_user_data(SipMsg1),
    ?assertError({error, no_user_data}, ersip_sipmsg:user_data(SipMsg2)),
    ok.
