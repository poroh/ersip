...
-type parse_result() :: {ok, addr()}
                      | {error, term()}.
-type raw_addr() :: {binary(), binary(), binary()}.

%%%===================================================================
%%% API
%%%===================================================================

-spec raw(addr()) -> raw_addr().
raw({ip4, IP4}) ->
...
-spec from_raw(raw_addr()) -> parse_result().
from_raw({NetType, AddrType, Addr}) when is_binary(NetType),
                                         is_binary(AddrType),
                                         is_binary(Addr) ->
    parse(NetType, AddrType, Addr).
