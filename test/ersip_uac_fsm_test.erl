...
-module(ersip_uac_fsm_test).

-include_lib("eunit/include/eunit.hrl").

uac_reliable_test() ->
    Tid = <<"transaction id">>,
    {UAC0, SideEffects0} = ersip_uac_fsm:new(Tid, reliable, message, #{}),
    ...
    %% ID is set:
    ?assertEqual(Tid, ersip_uac_fsm:id(UAC0)),

    %% Branch 1: (No reply at all)
    %% Check transaction timer fired:
    {UAC1, SideEffects_1_0} = ersip_uac_fsm:event({timer, TransactionTimer}, UAC0),
    ...
    ?assertEqual(timeout, ersip_uac_fsm:clear_reason(UAC1)),

    %% Branch 2: (Provisional response and no reply after):
    {UAC_2_0,SideEffects_2_0} = ersip_uac_fsm:event({resp, provisional, resp_msg}, UAC0),
    ...
    {UAC_2_1, SideEffects_2_1} = ersip_uac_fsm:event({timer, TransactionTimer}, UAC_2_0),
    SideEffectsMap_2_1 = maps:from_list(SideEffects_2_1),
    %% Transaction is cleared
    ?assertMatch(UAC_2_1, maps:get(clear_trans, SideEffectsMap_2_1)),
    ?assertEqual(timeout, ersip_uac_fsm:clear_reason(UAC_2_1)),

    %% Branch 3: (Two provisional responses and no final reply):
    {UAC_3_0, _SideEffects_3_0} = ersip_uac_fsm:event({resp, provisional, resp_msg}, UAC0),
    {UAC_3_1,  SideEffects_3_1} = ersip_uac_fsm:event({resp, provisional, resp_msg}, UAC_3_0),
    ...
    %% Transaction timer is fired
    {UAC_3_2, SideEffects_3_2} = ersip_uac_fsm:event({timer, TransactionTimer}, UAC_3_1),
    SideEffectsMap_3_2 = maps:from_list(SideEffects_3_2),
    %% Transaction is cleared
    ?assertMatch(UAC_3_2, maps:get(clear_trans, SideEffectsMap_3_2)),
    ?assertEqual(timeout, ersip_uac_fsm:clear_reason(UAC_3_2)),

    %% Branch 4: Final reply without provisional response:
    {UAC_4_0, SideEffects_4_0} = ersip_uac_fsm:event({resp, final, resp_msg}, UAC0),
    ...
    ?assertEqual(completed, ersip_uac_fsm:clear_reason(UAC_4_0)),

    %% Branch 5: Final reply after provisional response:
    {UAC_5_0, _SideEffects_5_0} = ersip_uac_fsm:event({resp, provisional, resp_msg}, UAC0),
    {UAC_5_1, SideEffects_5_1} = ersip_uac_fsm:event({resp, final, resp_msg}, UAC_5_0),
    ...
    ?assertEqual(completed, ersip_uac_fsm:clear_reason(UAC_5_1)).

uac_unreliable_test() ->
    Tid = <<"transaction unrel id">>,
    {UAC0, SideEffects0} = ersip_uac_fsm:new(Tid, unreliable, message, #{}),
    SideEffectsMap0 = maps:from_list(SideEffects0),
    %% New transaction is created:
    ?assertMatch(UAC0, maps:get(new_trans, SideEffectsMap0)),
    %% Message is sent
    ...
    %% Branch 1: (No reply at all)
    %% Retransmits #1
    {UAC_1_0, SideEffects_1_0} = ersip_uac_fsm:event({timer, RetransmitTimer}, UAC0),
    SideEffectsMap_1_0 = maps:from_list(SideEffects_1_0),
    ...
    %% Retransmits #2
    {UAC_1_1, SideEffects_1_1} = ersip_uac_fsm:event({timer, RetransmitTimer_1_0}, UAC_1_0),
    ...
    %% Check transaction timer fired:
    {UAC_1_2, SideEffects_1_2} = ersip_uac_fsm:event({timer, TransactionTimer}, UAC_1_1),
    SideEffectsMap_1_2 = maps:from_list(SideEffects_1_2),
    %% Transaction is cleared
    ?assertMatch(UAC_1_2, maps:get(clear_trans, SideEffectsMap_1_2)),
    ?assertEqual(timeout, ersip_uac_fsm:clear_reason(UAC_1_2)),
    ...
    {UAC_2_1, SideEffects_2_1} = ersip_uac_fsm:event({timer, RetransmitTimer_2_0}, UAC_2_0),
    SideEffectsMap_2_1 = maps:from_list(SideEffects_2_1),
    ...
    %% Retransmit #2
    {UAC_2_2, SideEffects_2_2} = ersip_uac_fsm:event({timer, RetransmitTimer_2_1}, UAC_2_1),
    ...
    %% Transaction timer is fired
    {UAC_2_3, SideEffects_2_3} = ersip_uac_fsm:event({timer, TransactionTimer}, UAC_2_2),
    SideEffectsMap_2_3 = maps:from_list(SideEffects_2_3),
    %% Transaction is cleared
    ?assertMatch(UAC_2_3, maps:get(clear_trans, SideEffectsMap_2_3)),
    ?assertEqual(timeout, ersip_uac_fsm:clear_reason(UAC_2_3)),
    ...
    {UAC_4_1, SideEffects_4_1} = ersip_uac_fsm:event({resp, final, resp_msg}, UAC_4_0),
    ...
    {UAC_4_2, SideEffects_4_2} = ersip_uac_fsm:event({timer, TimerK_4_0}, UAC_4_1),
    SideEffectsMap_4_2 = maps:from_list(SideEffects_4_2),
    ?assertMatch(UAC_4_2, maps:get(clear_trans, SideEffectsMap_4_2)),
    ?assertEqual(completed, ersip_uac_fsm:clear_reason(UAC_4_2)),
    ...
    {UAC_5_2, []} = ersip_uac_fsm:event({timer, RetransmitTimer}, UAC_5_1),
    {UAC_5_3, []} = ersip_uac_fsm:event({timer, TransactionTimer}, UAC_5_2),

    %% TimerK fired => transaction terminated
    {UAC_5_4, SideEffects_5_4} = ersip_uac_fsm:event({timer, TimerK_5_1}, UAC_5_3),
    SideEffectsMap_5_4 = maps:from_list(SideEffects_5_4),
    ?assertMatch(UAC_5_4, maps:get(clear_trans, SideEffectsMap_5_4)),
    ?assertEqual(completed, ersip_uac_fsm:clear_reason(UAC_5_4)).
